...
--***********************************************************************
CREATE OR REPLACE FUNCTION :schemaV.updatelrfield (  
  IN XIN TEXT)
  RETURNS VOID
...
--	  LASTMODTIMESTAMP TIMESTAMP PATH 'LASTMODTIMESTAMP',
  ...
  LOOP    
    UPDATE LRFIELD SET
      ENVIRONID=OP.ENVIRONID,
      LRFIELDID=OP.LRFIELDID,
      LOGRECID=OP.LOGRECID,
      NAME=OP.NAME,
      DBMSCOLNAME=OP.DBMSCOLNAME,
      FIXEDSTARTPOS=OP.FIXEDSTARTPOS,
      ORDINALPOS=OP.ORDINALPOS,
      ORDINALOFFSET=OP.ORDINALOFFSET,
...
--	   LASTMODTIMESTAMP          TIMESTAMP PATH 'LASTMODTIMESTAMP',
  ...
  LOOP    
    UPDATE LRFIELDATTR SET
      ...
      SORTKEYLABEL=OP.SORTKEYLABEL,
      INPUTMASK=OP.INPUTMASK,
      LASTMODTIMESTAMP=CURRENT_TIMESTAMP,
      LASTMODUSERID=OP.LASTMODUSERID
    WHERE ENVIRONID=OP.ENVIRONID
    AND LRFIELDID=OP.LRFIELDID;
