...
public class EnvironmentHandler extends AbstractHandler implements IHandler {

	public Object execute(ExecutionEvent event) throws ExecutionException {
		Environment env = SAFRApplication.getSAFRFactory().createEnvironment();
		EnvironmentEditorInput input = new EnvironmentEditorInput(env,
				EditRights.ReadModify);
		try {
			HandlerUtil.getActiveWorkbenchWindow(event).getActivePage()
					.openEditor(input, EnvironmentEditor.ID);
		} catch (PartInitException e) {
			UIUtilities.handleWEExceptions(e,"Unexpected error occurred while opening environment editor.",null);
		}
