...
public class DB2LRFieldDAO implements LRFieldDAO {
	static transient Logger logger = Logger
			.getLogger("com.ibm.safr.we.internal.data.dao.DB2LRFieldDAO");
	 ...
	 *            workbench.
	 */
	public DB2LRFieldDAO(Connection con, ConnectionParameters params,
	...
	public List<LRFieldTransfer> persistLRField(List<LRFieldTransfer> lrFieldTransfer) throws DAOException {
	...
	private List<LRFieldTransfer> createLRFields(List<LRFieldTransfer> lrFieldCreateList) throws DAOException {
		try {
		    ...
		    SAFRApplication.getTimingMap().startTiming("DB2LRFieldDAO.createLRField");
		    
			String statement = generator.getStoredProcedure(params.getSchema(), "INSLRFIELD", 2);
		...
		SAFRApplication.getTimingMap().stopTiming("DB2LRFieldDAO.createLRField");
    ...
    private List<LRFieldTransfer> updateLRField(List<LRFieldTransfer> lrFieldUpdate) throws DAOException {
        try {       
            SAFRApplication.getTimingMap().startTiming("DB2LRFieldDAO.updateLRField");
        ...
        SAFRApplication.getTimingMap().stopTiming("DB2LRFieldDAO.updateLRField");
        
        return lrFieldUpdate;        
    }
    
	public LRFieldTransfer persistLRField(LRFieldTransfer lrFieldTransfer) throws DAOException {
	...
	private LRFieldTransfer createLRField(LRFieldTransfer lrFieldTransfer) throws DAOException {
		try {
			String[] columnNames = { COL_ENVID, COL_ID, COL_LRID, 
			        COL_FIELDNAME, COL_DBMSCOLNAME, 
					COL_FIXEDSTARTPOS, COL_ORDINALPOS, COL_ORDINALOFFSET,
					COL_REDEFINE, COL_COMMENTS, COL_CREATETIME,
			...
			String statement = generator.getInsertStatementNoIdentifier(params.getSchema(),
					TABLE_LRFIELD, names, !lrFieldTransfer.isForImportOrMigration());
			...
			String[] columnNames1 = { COL_ENVID, COL_ID, COL_DATATYPE,
					COL_SIGNEDIND, COL_MAXLEN, COL_DECIMALCOUNT, COL_SCALING,
					COL_DATETIMEFORMAT, COL_HDRJUSTIFYCD, COL_HDRLINE1, COL_HDRLINE2, 
					COL_HDRLINE3, COL_SUBTLABEL, COL_SORTKEYLABEL, COL_INPUTMASK, 
					COL_CREATETIME, COL_CREATEBY, COL_MODIFYTIME, COL_MODIFYBY };
			List<String> names1 = Arrays.asList(columnNames1);

			String statement1 = generator.getInsertStatementNoIdentifier(
					params.getSchema(), TABLE_LRFIELD_ATTR, names1,
	...
	public void removeLRField(List<Integer> fieldIds, Integer environmentId) throws DAOException {
		try {
	...
	public List<LogicalRecordFieldQueryBean> queryAllLogicalRecordFields(SortType sortType, Integer environmentId, Integer logicalRecordId) throws DAOException {
	...
	private List<LogicalRecordFieldQueryBean> generateQueryBeanList(ResultSet rs, Integer environmentId) throws SQLException {
	...
	public List<LogicalRecordFieldQueryBean> queryAllLogicalRecordFields(SortType sortType, Integer environmentId) throws DAOException {
		String orderString = null;
		if (sortType.equals(SortType.SORT_BY_ID)) {
			orderString = "LRFIELDID";
		} else {
	...
	public List<LogicalRecordFieldQueryBean> queryLRFields(SortType sortType, Integer environmentId) throws DAOException {
		String orderString = null;
		if (sortType.equals(SortType.SORT_BY_ID)) {
			orderString = "LRFIELDID";
		} else {
    ...
    public Integer getNextKey() {
        try {
            String statement = "SELECT NEXT VALUE FOR " + params.getSchema() + 
                ".LRFIELD FROM SYSIBM.SYSDUMMY1";
            PreparedStatement pst = null;
            ResultSet rs = null;
            while (true) {
                try {
		...
		try {
			...
			while (true) {
				try {
					...
					pst.setString(3, fieldName.toUpperCase());
					rs = pst.executeQuery();
					break;
				} catch (SQLException se) {
					if (con.isClosed()) {
	...
	@Override
	public Integer getLookupLRField(Integer environID, int lookupid, String targetFieldName) {
		try {
			Integer result = null;
			String schema = params.getSchema();
			...
			String selectString = "select lrfieldid from " + schema + ".lrfield f"
					...
					+ " where l.lookupid = ? and UPPER(f.name) = ? and f.environid = ?";
			...
			while (true) {
				try {
					...
					pst.setString(2, targetFieldName.toUpperCase());
