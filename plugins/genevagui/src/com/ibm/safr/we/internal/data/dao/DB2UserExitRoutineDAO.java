 ...
 */
public class DB2UserExitRoutineDAO implements UserExitRoutineDAO {

	static transient Logger logger = Logger
			.getLogger("com.ibm.safr.we.internal.data.dao.DB2UserExitRoutineDAO");
	 ...
	 *            workbench.
	 */
	public DB2UserExitRoutineDAO(Connection con, ConnectionParameters params,
	...
	public List<UserExitRoutineQueryBean> queryUserExitRoutines(
		...
		try {
			...
			if (sortType.equals(SortType.SORT_BY_ID)) {
				orderString = "A.EXITID";
			} else if (sortType.equals(SortType.SORT_BY_NAME)) {
				orderString = "UPPER(A.NAME)";
			}  else if (sortType.equals(SortType.MODULE_NAME)) {
				orderString = "UPPER(A.MODULEID)";			
			}
	...
	public Map<ComponentType, List<DependentComponentTransfer>> getUserExitRoutineDependencies(
		...
		try {
          ...
          // Getting Views dependent on specified User Exit Routine in output logic.Check whether the user exit routine is present in the view source table.
          selectDependentViews = "SELECT DISTINCT A.VIEWID, A.NAME FROM " +
              params.getSchema() + ".VIEW A, " +
              params.getSchema() + ".VIEWSOURCE C " +
              "WHERE A.ENVIRONID=C.ENVIRONID " +
              "AND A.VIEWID=C.VIEWID " +
              "AND C.WRITEEXITID=? AND A.ENVIRONID=? UNION " +
              "SELECT DISTINCT A.VIEWID, A.NAME FROM " +
              params.getSchema() + ".VIEW A, " +
              params.getSchema() + ".VIEWLOGICDEPEND C " +
              "WHERE A.ENVIRONID=C.ENVIRONID " +
              "AND A.VIEWID=C.VIEWID " +
              "AND C.EXITID=? AND A.ENVIRONID=?";
            ...
            while (true) {
                try {
                    pst = con.prepareStatement(selectDependentViews);
                    pst.setInt(1, userExitRoutineId);
                    pst.setInt(2, environmentId);
                    pst.setInt(3, userExitRoutineId);
	...
	public Integer getUserExitRoutine(String name, Integer environmentId, boolean procedure) {
		...
		try {
			String selectString = "Select EXITID FROM "
					+ schema + ".EXIT e where e.environid= ? ";
					if(procedure) {
						selectString += " and UPPER(e.MODULEID) = ?;";						
					} else {
						selectString += " and UPPER(e.name) = ?;";
					}
			PreparedStatement pst = null;
			ResultSet rs = null;
			while (true) {
