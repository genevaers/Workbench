...
public class PGReportsDAO implements ReportsDAO {
	...
	public PGReportsDAO(Connection con, ConnectionParameters params,
        ...
        try {
            ...
            while (true) {
                try {
                	...
                	pst.setInt(1,  environmentId);
                	pst.setInt(2,  viewId);
                	pst.setInt(3, Integer.valueOf(sourceNum));
                    rs = pst.executeQuery();
                    break;
                } catch (SQLException se) {
                    if (con.isClosed()) {
	...
	public List<ViewMappingsCSVReportQueryBean> getViewColumnCSVMappings(List<Integer> viewIds, Integer environmentId) throws DAOException {
        ...
        try {
            String selectString =		"SELECT C.VIEWID,"
            		...
            		+ "		ON K.ENVIRONID=C.ENVIRONID AND K.VIEWCOLUMNID=C.VIEWCOLUMNID "
            		+ "		LEFT JOIN " + params.getSchema() + ".LRFIELD T "
            		+ "		ON T.ENVIRONID=C.ENVIRONID AND T.LRFIELDID=C.SORTTITLELRFIELDID "
