...
public class TestLogicalRecord extends TestCase {
	...
	public void testGetLRTypeCode() {
		dbStartup();
		LogicalRecord lr = SAFRApplication.getSAFRFactory()
				.createLogicalRecord();
		Code LRTypeCode = null;
		CodeSet codeSet;
		try {
			codeSet = SAFRApplication.getSAFRFactory().getCodeSet(
					CodeCategories.LRTYPE);
			assertEquals(lr.getLRTypeCode(), null);
		...
		try {
			codeSet = SAFRApplication.getSAFRFactory().getCodeSet(
					CodeCategories.LRTYPE);
			LRTypeCode = codeSet.getCode("FILE");
			lr.setLRTypeCode(LRTypeCode);
			assertEquals(lr.getLRTypeCode(), LRTypeCode);
			noException = true;
		} catch (SAFRException e) {
			throw new RuntimeException(e);
		}
		...
		// test for null LR Type Code
		boolean correctException = false;
		try {
			lr.setLRTypeCode(null);
	...
	public void testGetLRStatusCode() {
		dbStartup();
		LogicalRecord lr = SAFRApplication.getSAFRFactory()
				.createLogicalRecord();
		Code LRStatusCode = null;
		CodeSet codeSet;
		try {
			assertEquals(lr.getLRStatusCode(), null);
		} catch (SAFRException e) {
			throw new RuntimeException(e);
		}
		boolean noException = false;
		try {
			...
			LRStatusCode = codeSet.getCode("ACTVE");
			lr.setLRStatusCode(LRStatusCode);
			assertEquals(lr.getLRStatusCode(), LRStatusCode);
			noException = true;
		} catch (SAFRException e) {
			throw new RuntimeException(e);
		}
		...
		// test for null LR Status Code
		boolean correctException = false;
		try {
			lr.setLRStatusCode(null);
		} catch (NullPointerException npe) {
			correctException = true;
		}
		assertTrue(correctException);
	...
	public void testStore() throws SAFRException {
		...
		ComponentAssociation componentAssociation1 = null;
		ComponentAssociation componentAssociation2 = null;
		LogicalRecord lr = SAFRApplication.getSAFRFactory()
				.createLogicalRecord();
		...
		componentAssociation = new ComponentAssociation(lr, 5, "jaydeep",null);
		componentAssociation1 = new ComponentAssociation(lr, 3,"test_jaydeep", null);
		componentAssociation2 = new ComponentAssociation(lr, 4,"test_jayd_21", null);
		...
		lr.getLogicalFileAssociations().add(componentAssociation1);
		lr.getLogicalFileAssociations().add(componentAssociation2);
		lr.store();
        delIds.add(lr.getId());
		PGDAOFactory fact = null;
		fact = (PGDAOFactory) DAOFactoryHolder.getDAOFactory();

		assertEquals(lr.getName(), "lr12");
		assertEquals(lr.getComment(), "test");
		assertEquals(lr.getCreateBy(), fact.getSAFRLogin().getUserId());
		assertEquals(lr.getLogicalFileAssociations().get(2),componentAssociation);
		assertEquals(lr.getLogicalFileAssociations().get(0),componentAssociation1);
		assertEquals(lr.getLogicalFileAssociations().get(1),componentAssociation2);
	...
	public void testValidateDependencyError() throws SAFRException {
		dbStartup();
		helper.setEnv(ENVIRONID);
		
		LogicalRecord lr = SAFRApplication.getSAFRFactory().getLogicalRecord(1491);

		SAFRList<LRField> lrf_lists = lr.getLRFields();
		LRField lrf = lrf_lists.get(0);
	
		lrf.setPersistence(SAFRPersistence.DELETED);
		
	
	        try {
	            lr.validate();
            } catch (SAFRValidationException e) {
            	System.out.print(e.getMessageString());
            	assertEquals("LOOKUP PATHS :" + SAFRUtilities.LINEBREAK + "    1. Field ID[91902]" + SAFRUtilities.LINEBREAK + "        ->LF_test_LR_Validate_Error [2112]",e.getMessage().substring(0, 82));
            	assertEquals("VIEWS :" + SAFRUtilities.LINEBREAK + "    1. Field ID[91902]" + SAFRUtilities.LINEBREAK + "        ->V_test_LR_Validate_Error [8717]" + SAFRUtilities.LINEBREAK,e.getMessage().substring(84));
            }
	}
	
	public void testSaveAsNoPerm() throws DAOException, SAFRException {
		dbStartup();
		helper.setEnv(ENVIRONID);
		helper.setUser("NOPERM");
	
		LogicalRecord lr1 = SAFRApplication.getSAFRFactory().getLogicalRecord(1490);
		try {
	        lr1.saveAs("test");
	        ...
	        e.printStackTrace();
	        assertEquals("The user is not authorized to create a Logical Record.",e.getMessage());
        }
	}
