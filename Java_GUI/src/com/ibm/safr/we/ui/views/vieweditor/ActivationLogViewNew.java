...
public class ActivationLogViewNew extends ViewPart {

    public static String ID = "SAFRWE.ActivationLogViewNew";
	...
	public void showGridForCurrentEditor(IEditorPart editor) {
		...
		if ((editor instanceof ViewEditor)
			...
			if (editor instanceof ViewEditor) {
				...
				ViewActivator.getNewCompilerVersion() + ")");
				label.setText("Activation Log unavailable");
				if (vEditor.viewActivationMessageExistsNew()) {
					this.viewActivationLog = vEditor.getViewActivationException();
				}
			} else if (editor instanceof LogicTextEditor) {
                logicTextEditor = (LogicTextEditor) editor;
                setPartName("Logic Text Validation Log (" + 
                ViewActivator.getNewCompilerVersion() + ")");			    
				label.setText("Logic Text Validation Log unavailable");
				if (logicTextEditor.isLogicTextValidationMessageExistsNew()) {
				...
				this.setPartName("View Activation Log (" + 
				ViewActivator.getNewCompilerVersion() +")");
				label.setText("Activation Log unavailable");
			...
			if (viewActivationLog != null) {
				for (ViewActivationError error : viewActivationLog.getActivationLogNew()) {
