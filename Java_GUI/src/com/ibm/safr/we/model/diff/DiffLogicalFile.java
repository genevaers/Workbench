...
public class DiffLogicalFile extends DiffNodeComp {

    private LogicalFile lhs;
    private LogicalFile rhs;
    
    
    public DiffLogicalFile(LogicalFile lhs, LogicalFile rhs) {
        super();
        this.lhs = lhs;
        this.rhs = rhs;
    }
    ...
    protected DiffNode generatePhysicalFiles() throws SAFRException {
        ...
        for (FileAssociation fal : lhs.getPhysicalFileAssociations()) {
            if (fal.getAssociatedComponentIdNum() != 0) {
                PhysicalFile lhsComp = SAFRApplication.getSAFRFactory().getPhysicalFile(fal.getAssociatedComponentIdNum(), fal.getEnvironmentId());
                lhsList.add(lhsComp);
            }
        }

        List<SAFREnvironmentalComponent> rhsList = new ArrayList<SAFREnvironmentalComponent>();
        for (FileAssociation far : rhs.getPhysicalFileAssociations()) {
            if (far.getAssociatedComponentIdNum() != 0) {
                PhysicalFile rhsComp = SAFRApplication.getSAFRFactory().getPhysicalFile(far.getAssociatedComponentIdNum(), far.getEnvironmentId());
                rhsList.add(rhsComp);
            }
        }
        
        List<DiffFieldReference> refList = diffReferences(MetaType.PHYSICAL_FILES, "Physical File", lhsList, rhsList, 
    ...
    protected void generateTree() throws SAFRException {
        DiffLogicalFile tree = (DiffLogicalFile) getGenerated(DiffLogicalFile.class, lhs.getId());
        if (tree == null) {
            setId(lhs.getId());
            ...
            if (gi.getState() == DiffNodeState.Same && pfs.getState() == DiffNodeState.Same) {
            ...
            storeGenerated(DiffLogicalFile.class, lhs.getId(), this);
