...
public class Grid extends Canvas {
    ...
    private Rectangle prevSize = null;
    
    /**
     * Flag indicates we need to show the selected column 
     * after a grid resize
     */
    private boolean hScrollShowCol = false;
	...
	public Grid(DataVisualizer dataVisualizer, Composite parent, int style) {
        ...
        addControlListener(new ControlListener() {
            ...
            public void controlResized(ControlEvent e) {
                Rectangle gridR =  getClientArea();
                
                // if prevSize exists and new width is smaller and a cell is selected
                if (prevSize != null &&
                    gridR.width < prevSize.width &&
                    getFocusItem() != null && 
                    getFocusCell().x >=0 &&
                    getFocusCell() != null) {
                    
                    int diffW = prevSize.width - gridR.width;
                    Rectangle diffR = new Rectangle(gridR.width, gridR.y, diffW, gridR.height);
                    Rectangle cellR =  getFocusItem().getBounds(getFocusCell().x);

                    // if the difference intersects with the focus cell
                    // i.e. the focus cell is being covered then we 
                    // should show it
                    if (diffR.intersects(cellR)) {
                        // need to show column after updating scroll bars
                        hScrollShowCol = true;
                    }
                    else {
                        hScrollShowCol = false;
                        prevSize = getClientArea();
                    }                    
                }
                else {
                    hScrollShowCol = false;                    
	...
	private int getHScrollSelectionInPixels() {
		...
		if (columnScrolling) {
			...
			for (int i = 0; i < selection; i++) {
			    if(displayOrderedColumns.size() > i)
			    {
	...
	private void updateScrollbars() {
		...
		if (hScroll.getVisible()) {
			...
			if (!columnScrolling) {
				...
				int visCols = 0;
                int disCols = 0;
                Rectangle gRec= new Rectangle(getItemHeaderWidth(), clientArea.y, 
                        clientArea.width-getItemHeaderWidth(), clientArea.height);				
				for (Iterator iter = columns.iterator(); iter.hasNext();) {
					GridColumn element = (GridColumn) iter.next();
					if (element.isVisible()) {
						visCols++;
					}
                    Rectangle cRec = element.getBounds();
                    if (gRec.intersection(cRec).equals(cRec)) {
                        disCols++;
                    }					
				}
                ...
                hScroll.setValues(selection, 0, max, 1, 1, disCols);
				
                if (hScrollShowCol) {
                    showColumn(getColumn(getFocusCell().x));
                    hScrollShowCol = false;
                }
	...
	private boolean handleCellHover(int x, int y) {
		...
		if (hoverChange || hoveringOverText != overText) {
			...
			if (overText) {
				...
				if (hoveringItem != null &&  !col.getWordWrap()) //dont show inplace tooltips for cells with wordwrap
				{
